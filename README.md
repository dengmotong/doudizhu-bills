...
5. **已知玩家注入**：v1.0 新增，将系统中已有玩家列表注入 prompt，引导 LLM 优先匹配现有昵称，避免 -、—、一 等相似字符导致的识别偏差
...
- **识别优化**：AI 识别时注入已知玩家列表，大幅减少因 -、—、一 等相似字符导致的昵称误识别
- **统计修复**：修复按日统计日期错位问题，图表改用密集排列消除空隙；月度/每日趋势改为累计趋势；0 轴线居中显示
- **日期中文化**：所有日期显示改为中文格式（2024年1月15日）
- **文档完善**：新增 `AGENTS.md` 帮助 AI 代理快速上手项目
